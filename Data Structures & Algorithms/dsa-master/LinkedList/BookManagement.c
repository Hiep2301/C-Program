...
int deleteByIsbn(ListBook **first, char *ibn)
{
    ...
    if (current != NULL)
    {
        ...
        if (current == pre)
        {
            *first = current->next; // gán node sau là node đầu danh sách
        }
        else
        {
            pre->next = current->next; // gán con trỏ next node của đằng trước node cần xóa vào node ở đằng sau node cần xóa
        }
